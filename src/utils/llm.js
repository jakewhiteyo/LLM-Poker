...
const handleLLM = async (state, pushAnimationState) => {
  ...
  const testResponse =
    "**Raise 40** -  You have a strong hand with Queen's and want to encourage other players to fold while also increasing the pot. ";
...
const handleResponse = (response, state) => {
  ...
  if (action.startsWith("check")) {
    ...
    if (parsedAmount > state.minBet) {
      return handleBet(state, parsedAmount, min, max);
    } else {
